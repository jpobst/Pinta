...
/// <summary>
/// Histogram is used to calculate a histogram for a pixel buffer (in a selection,
...
public abstract class Histogram
{
	...
	public long[][] HistogramValues {
		...
		set {
			if (value.Length != histogram.Length || value[0].Length != histogram[0].Length)
				throw new ArgumentException ("value must be an array of arrays of matching size", nameof (value));
	...
	protected abstract void AddPixelBufferRectangleToHistogram (PixelBuffer surface, RectangleI rect);

	public void UpdateHistogram (PixelBuffer surface, RectangleI rect)
	{
		Clear ();
		AddPixelBufferRectangleToHistogram (surface, rect);
		OnHistogramUpdated ();
	}
}
