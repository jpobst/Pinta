		...
		if (AlphaOnly && AlphaBlending) {
			for (var x = rect.Left; x <= right; ++x) {
				...
				row[x] = ColorBgra.FromBgra (
					b: FoundationUtility.FastScaleByteByByte (originalPixel.B, lerpAlpha),
					g: FoundationUtility.FastScaleByteByByte (originalPixel.G, lerpAlpha),
					r: FoundationUtility.FastScaleByteByByte (originalPixel.R, lerpAlpha),
					a: FoundationUtility.FastScaleByteByByte (originalPixel.A, lerpAlpha));
			}
